(function () {
   ...
   angular.module('app.PVIOperationsDemoCtrl', ['app.service-core', 'app.service-user', 'app.service-operationmanager', 'app.service-fieldsmanager', 'app.service-operationstore', 'app.document-groups', 'app.doc-entry-configuration', 'app.validation-configuration', 'app.validation-manager']);

   function PVIOperationsDemoCtrl($q, $scope, $filter, $location, $translate, $timeout, $interval,
    ...
    self.identityConfiguration = {
      Dni : DocEntryConfiguration.getDocEntryConfiguration('Dni', 'PVIApp'),
      NIE : DocEntryConfiguration.getDocEntryConfiguration('NIE', 'PVIApp'),
      Passport : DocEntryConfiguration.getDocEntryConfiguration('Passport', 'PVIApp'),
      Not_Recognized_Identity : DocEntryConfiguration.getDocEntryConfiguration('Not_Recognized_Identity', 'PVIApp')
    };

    self.incomeConfiguration = {
      Paysheet : DocEntryConfiguration.getDocEntryConfiguration('Paysheet', 'PVIApp'),
      IRPF : DocEntryConfiguration.getDocEntryConfiguration('IRPF', 'PVIApp'),
      Not_Recognized_Income : DocEntryConfiguration.getDocEntryConfiguration('Not_Recognized_Income', 'PVIApp')
    };

    self.receiptConfiguration = {
      Bank_Receipt : DocEntryConfiguration.getDocEntryConfiguration('Bank_Receipt', 'PVIApp'),
      Receipt : DocEntryConfiguration.getDocEntryConfiguration('Receipt', 'PVIApp'),
      Not_Recognized_Receipt : DocEntryConfiguration.getDocEntryConfiguration('Not_Recognized_Receipt', 'PVIApp')
    };

    self.identityValidation = {
      Dni : ValidationConfiguration.getValidationConfiguration('Dni','PVIApp'),
      NIE : ValidationConfiguration.getValidationConfiguration('NIE','PVIApp'),
      Passport : ValidationConfiguration.getValidationConfiguration('Passport','PVIApp'),
      Not_Recognized_Identity : ValidationConfiguration.getValidationConfiguration('Not_Recognized_Identity','PVIApp')
    };

    self.incomeValidation = {
      Paysheet : ValidationConfiguration.getValidationConfiguration('Paysheet','PVIApp'),
      IRPF : ValidationConfiguration.getValidationConfiguration('IRPF','PVIApp'),
      Not_Recognized_Income : ValidationConfiguration.getValidationConfiguration('Not_Recognized_Income','PVIApp')
    };

    self.receiptValidation = {
      Bank_Receipt : ValidationConfiguration.getValidationConfiguration('Bank_Receipt','PVIApp'),
      Receipt : ValidationConfiguration.getValidationConfiguration('Receipt','PVIApp'),
      Not_Recognized_Receipt : ValidationConfiguration.getValidationConfiguration('Not_Recognized_Receipt','PVIApp')
    };
    ...
    self.opStatus = '';
    self.opUserAuthorization = false;
    ...
    self.checkPVIReady = OperationManager.checkPVIReady;
    ...
    self.sendToPVI = function() {
      var deferred = $q.defer();
      OperationManager.sendToPVI()
      .then(function(){
        self.refresh();
        $scope.startQuery();
        deferred.resolve();
        self.pvimodal = false;
        self.showInfo('PVI_SUCCESFULL');
      },function(){
        $scope.startQuery();
        self.pvimodal = false;
    ...
    self.setServerResponseError = function(){
      self.errorCallBack('SERVER_RESPONSE_ERROR');
    };
    ...
    self.updateStat = function(callback){
      OperationManager.getOpStat().then(
        function(opstat){
          ...
          self.opStatus = opstat.documentStatus;
          var doctype = DocumentGroups.getSelectedType('Identity').doctype;
          var fields = self.identityConfiguration[doctype].fields;
           OperationManager.updateAuthorization(fields).then(
             function(){
             },
             function(){
            ...
            if (OperationStore.getOp().attributes.operation_user_authorization === '1') {
              self.opUserAuthorization = true;
            } else {
              self.opUserAuthorization = false;
            }
    ...
    self.updateSelected = function(oldtype, doc){
      ...
      if(group === 'Identity'){
        ...
        self.updateAttributesValuesOperation(doc, self.selectedIdentity);
      }
      if(group === 'Income'){
    ...
    self.updateAttributesValuesOperation = function(doc, selected){
      Object.keys(doc.attributes).forEach(function() {
        for (var i = 0; i < selected.fields.length; i++) {
          if (selected.fields[i].id === 'authorization') {
            self.updateAttributeValueOperation(selected.fields[i]);
          }
        }
      });
    };

    self.updateAttributeValueOperation = function(field){
      field.value = self.opUserAuthorization;
    };
    ...
    self.updateAttributeValue = function(key, field, doc){
      ...
      if (field.controltype === 'date'){
        ...
        field.value = parseFloat(doc.attributes[key]).toString();
      } else {
      ...
      OperationManager.finishOp().then(
        function(){
          if((self.terminated===true &&
              self.opStatus === 'OK' && self.opUserAuthorization)) {
            $scope.stopQuery();
          }
    ...
    self.isSendPVIButtonDisabled = function() {
  ...
  PVIOperationsDemoCtrl.$inject=['$q', '$scope', '$filter', '$location', '$translate','$timeout', '$interval',
                              'NgTableParams', 'PageStatus', 'OperationManager','FieldsManager', 'OperationStore', 'DocumentGroups', 'DocEntryConfiguration', 'ValidationConfiguration', 'ValidationManager'];
  angular
  .module('app.PVIOperationsDemoCtrl')
  .controller('PVIOperationsDemoCtrl', PVIOperationsDemoCtrl);
