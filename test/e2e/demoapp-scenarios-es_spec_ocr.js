...
describe('ValidationDemoApp', function() {
  var login_page;
  var validation_page;
  var waitTimeOut = 36000;

  var dni_resource = './resources/ocr/dni.jpg';
  var receipt_resource = './resources/ocr/recibo_sin_nombre.jpg';
  var paysheet_resource = './resources/ocr/nomina_con_nombre.jpg';
  var paysheet_amount = '1.686,04';
  ...
  describe('Validation page', function() {
    ...
    /*******************************************************************************************************************
      ...
      a) all test results are based on the real OCR plugin named 'OCRPluginDefault', same as groovy scripts install.

      b) before init test, modify the configuration file config.json to set the url to the OCR service machine.
         p.e. "restUrl": "http://10.0.3.60:8080/IDocValidationService/resources"

      c) DO NOT commit this modified config.json file to svn repository
    ...
    ********************************************************************************************************************/
    ...
    it('should set an image to Dni drop area and show ocr-test recognition', function() {
      ...
      expect(inputList.get(0).getAttribute('value')).toEqual('DIANA');
      expect(inputList.get(1).getAttribute('value')).toEqual('GIL GALAN');
      expect(inputList.get(2).getAttribute('value')).toEqual('49015899R');
      expect(inputList.get(3).getAttribute('value')).toEqual('27/05/2018');
    ...
    it('should set an image to every drop area and show all complete results', function() {
      ...
      absolutePath = path.resolve(__dirname, receipt_resource);
      ...
      elementPanelTitleReceipt.getText().then(function(text){expect(text).toContain("Justificantes Recibo")});
      ...
      expect(checkboxPanelJustificanteBancarioKO.count()).toEqual(2);
      ...
      expect(checkboxPanelJustificanteBancarioOK.count()).toEqual(1);
      ...
      expect(checkboxPanelNominaKO.count()).toEqual(0);
      var checkboxPanelNominaOK = elementPanelIncome.all(by.css('.glyphicon-ok'));
      expect(checkboxPanelNominaOK.count()).toEqual(3);
      ...
      expect(checkboxDocumentAllKO.count()).toEqual(1);
    ...
    it('should set a Receipt when automatic is selected and OCR return Receipt in Receipt', function() {
      ...
      validation_page.setAutomaticAccount('');
      absolutePath = path.resolve(__dirname, receipt_resource);
      ipsaImgList.get(1).sendKeys(absolutePath);
